...
- **Leaflet Tile Servers (Critical)**: Do NOT use OpenStreetMap tiles (causes 403 errors in sandbox). You MUST use Google Maps tiles, AND you MUST implement a working layer control UI with 3 switchable layers. Use this exact implementation pattern:
  ```javascript
  const googleTerrain = L.tileLayer('https://mt1.google.com/vt/lyrs=p&hl={lang}&x={x}&y={y}&z={z}', { maxZoom: 20 });
  const googleSatellite = L.tileLayer('https://mt1.google.com/vt/lyrs=y&hl={lang}&x={x}&y={y}&z={z}', { maxZoom: 20 });
  const googleStreets = L.tileLayer('https://mt1.google.com/vt/lyrs=m&hl={lang}&x={x}&y={y}&z={z}', { maxZoom: 20 });
  const baseMaps = { "Terrain": googleTerrain, "Satellite": googleSatellite, "Streets": googleStreets };
  googleTerrain.addTo(mapInstance); // Default layer
  L.control.layers(baseMaps, null, { position: 'topleft' }).addTo(mapInstance);
  ```
  Replace `{lang}` with `'iw'` for Hebrew or `'en'` for English per Language Policy. Position `topleft` avoids RTL scrollbar overlap.
...
| [CA-HE] | Hebrew terminology map for structural output elements | When user interacts in Hebrew |
